...
void level_wipe(int wipe_ents)
{
    ...
    // get original (texture/sound/instrument) chunks
    ...
    for (int i = 0; i < entry_count; i++)
    {
        ...
        if (elist[i].og_chunk > chunk_count)
            chunk_count = elist[i].og_chunk + 1;
    }
    ...
    printf("\nSaved as %s / .NSD\n", nsfpathout);
    printf("NSD patch with CrashEdit recommended\n");
    printf("Done.\n\n");
}