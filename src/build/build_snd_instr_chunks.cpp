...
void build_instrument_chunks(ELIST& elist, int32_t* chunk_count, uint8_t** chunks)
{
	...
	for (auto& ntry : elist)
	{
		...
		*(uint32_t*)(chunks[chunk_index] + 0x14) = 0x24 + ntry.esize;

		memcpy(chunks[chunk_index] + 0x24, ntry._data(), ntry.esize);
...
void build_sound_chunks(ELIST& elist, int32_t* chunk_count, uint8_t** chunks)
{
	...
	for (int32_t i = 0; i < snd_chunk_count; i++)
	{
		...
		*(uint16_t*)(chunks[start_chunk_idx + i] + 8) = local_entry_count;
		...
		{
			if (sound_list[j].chunk == start_chunk_idx + i)
				memcpy(chunks[start_chunk_idx + i] + offsets[indexer++], sound_list[j]._data(), sound_list[j].esize);
		}
