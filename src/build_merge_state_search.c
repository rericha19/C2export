...
void build_premerge_anim_model(ENTRY *elist, int32_t entry_count, int32_t chunk_border_sounds, int32_t *chunk_count, int32_t *config, LIST permaloaded)
{
    LIST temp_entries = build_get_normal_entry_list(elist, entry_count);
    LIST anims_models = init_list();
    for (int32_t i = 0; i < temp_entries.count; i++)
    {
        int32_t entry_type = build_entry_type(elist[build_get_index(temp_entries.eids[i], elist, entry_count)]);
    ...
    int32_t **entry_matrix = build_get_occurence_matrix(elist, entry_count, anims_models, config);
    RELATIONS relation_array = build_transform_matrix(anims_models, entry_matrix, config, elist, entry_count);
    for (int32_t i = 0; i < anims_models.count; i++)
        free(entry_matrix[i]);
    free(entry_matrix);
    ...
    for (int32_t i = 0; i < entry_count; i++)
    {
        if (build_entry_type(elist[i]) == ENTRY_TYPE_MODEL)
        {
            ...
            for (int32_t j = 0; j < entry_count; j++)
                ...
                {
                    int32_t potential_combined_size = 0x14 + 4 * 2 + elist[i].esize + elist[j].esize;
                    if (potential_combined_size > CHUNKSIZE)
                        continue;

                    uint32_t model_ref = build_get_model(elist[j].data, 0);
                    if (model_ref != elist[i].eid)
                        continue;

                    uint32_t anim_eid = elist[j].eid;
            ...
            int32_t max_value = -1;
            int32_t chunk_index1 = 0;
            int32_t chunk_index2 = 0;

            for (int32_t j = 0; j < relation_array.count; j++)
            {
                uint32_t entry1 = anims_models.eids[relation_array.relations[j].index1];
                uint32_t entry2 = anims_models.eids[relation_array.relations[j].index2];
                int32_t value = relation_array.relations[j].value;
...
// assumes one entry per chunk, ignores permaloaded entries
void build_premerge_slst_zone(ENTRY *elist, int32_t entry_count, int32_t chunk_border_sounds, int32_t *chunk_count, int32_t *config, LIST permaloaded)
{
    ...
    for (int32_t i = 0; i < temp_entries.count; i++)
    {
        int32_t index = build_get_index(temp_entries.eids[i], elist, entry_count);
        int32_t entry_type = build_entry_type(elist[index]);
        if (entry_type == ENTRY_TYPE_ZONE || entry_type == ENTRY_TYPE_SLST)
            if (list_find(permaloaded, temp_entries.eids[i] == -1))
                list_add(&zones_slsts, temp_entries.eids[i]);
    }

    int32_t **entry_matrix = build_get_occurence_matrix(elist, entry_count, zones_slsts, config);
    RELATIONS relation_array = build_transform_matrix(zones_slsts, entry_matrix, config, elist, entry_count);
    for (int32_t i = 0; i < zones_slsts.count; i++)
        free(entry_matrix[i]);
    free(entry_matrix);
    ...
    for (int32_t i = 0; i < entry_count; i++)
    {
        if (build_entry_type(elist[i]) == ENTRY_TYPE_ZONE)
        {
            ...
            {
                uint32_t offset = build_get_nth_item_offset(elist[i].data, 2 + 3 * j);
                uint32_t slst = build_get_slst(elist[i].data + offset);
                int32_t slst_index = build_get_index(slst, elist, entry_count);
                if (slst_index == -1)
                    continue;

                int32_t potential_combined_size = 0x14 + 4 * 2 + elist[i].esize + elist[slst_index].esize;
                if (potential_combined_size > CHUNKSIZE)
                    continue;
            ...
            for (int32_t j = 0; j < relation_array.count; j++)
            {
                uint32_t entry1 = zones_slsts.eids[relation_array.relations[j].index1];
                uint32_t entry2 = zones_slsts.eids[relation_array.relations[j].index2];
                int32_t value = relation_array.relations[j].value;
...
void build_state_search_premerge(ENTRY *elist, int32_t entry_count, int32_t chunk_border_sounds, int32_t *chunk_count, int32_t *config, LIST permaloaded)
{
    int32_t type = 0;
 ...
 * \param chunk_border_sounds int32_t       index of the first normal chunk
...
void build_merge_state_search_main(ENTRY *elist, int32_t entry_count, int32_t chunk_border_sounds, int32_t *chunk_count, int32_t *config, LIST permaloaded)
{
    ...
    int32_t perma_chunk_count = build_permaloaded_merge(elist, entry_count, chunk_border_sounds, chunk_count, permaloaded);
    int32_t first_nonperma_chunk_index = *chunk_count;
 ...
 *
 * \param length int32_t                    amount of entry-chunk assignments
...
STATE_SEARCH_STR *build_state_search_str_init(int32_t length)
{
    ...
    temp->estimated = 0;
    temp->elapsed = 0;
    temp->entry_chunk_array = (uint16_t *)malloc(length * sizeof(uint16_t)); // freed by caller's caller (i hope)
    return temp;
}
                                           ...
                                           int32_t first_nonperma_chunk, int32_t perma_count, int32_t max_payload_limit, int32_t *max_pay)
{
    ...
    int32_t chunk_count = build_state_search_str_chunk_max(state, key_length) + 1;
...
int32_t chunk_size_sort_cmp(const void *a, const void *b)
{
 ...
 *
 * \param state A_STAR_STR*             input state
 * \param chunk1 uint32_t           index of first chunk
 * \param chunk2 uint32_t           index of second chunk (higher than chunk1)
 * \param key_length int32_t              amount of involved entries
 * \return A_STAR_STR*                  new state
 */
STATE_SEARCH_STR *build_state_search_merge_chunks(STATE_SEARCH_STR *state, uint32_t chunk1, uint32_t chunk2,
                                                  int32_t key_length, int32_t first_nonperma_chunk, ENTRY *temp_elist)
{
    int32_t i;
    ...
    int32_t resulting_size = 0x14;
    for (i = 0; i < key_length; i++)
    {
        uint16_t curr_chunk = state->entry_chunk_array[i];
    ...
    /*
    int32_t chunk_count = 1 + build_state_search_str_chunk_max(new_state, key_length);
    ...
    for (i = 0; i < key_length; i++) {
        int32_t chunk_index = new_state->entry_chunk_array[i];
    ...
    for (i = 0; i < key_length; i++) {
        int32_t curr_chunk = new_state->entry_chunk_array[i];
 ...
 * \param elist ENTRY*                  entry list
 * \param entry_count int32_t               entry count
 * \param first_nonperma_chunk_index int32_t         chunk where involved entries start
 * \param key_length int32_t              amount of involved entries
...
int32_t build_state_search_is_empty_chunk(STATE_SEARCH_STR *state, uint32_t chunk_index, int32_t key_length)
{
    ...
    int32_t has_something = 0;
    for (int32_t k = 0; k < key_length; k++)
    {
...
uint32_t *build_state_search_init_elist_convert(ENTRY *elist, int32_t entry_count, int32_t first_nonperma_chunk_index, int32_t *key_length)
{
    ...
    uint32_t *eid_list = (uint32_t *)malloc(counter * sizeof(uint32_t)); // freed by caller
...
LIST *build_state_search_eval_util(ENTRY *elist, int32_t entry_count, int32_t first_nonperma_chunk, ENTRY **temp_elist, int32_t *key_length, int32_t *ll_snap_count)
{

    int32_t lcl_key_length = 0;
    ;
    // also gets key length
    uint32_t *eid_list = build_state_search_init_elist_convert(elist, entry_count, first_nonperma_chunk, &lcl_key_length);
    ENTRY *local_temp_elist = (ENTRY *)malloc(lcl_key_length * sizeof(ENTRY));

    for (int32_t i = 0; i < lcl_key_length; i++)
    {
        local_temp_elist[i].eid = eid_list[i];
        int32_t master_elist_index = build_get_index(eid_list[i], elist, entry_count);
            ...
            for (j = 0; j < cam_count; j++)
            {
                ...
                for (l = 0; l < load_list.count; l++)
                {
                    if (load_list.array[l].type == 'A')
                        ...
                        {
                            uint32_t eid = load_list.array[l].list[m];
                            int32_t index = build_get_index(eid, local_temp_elist, lcl_key_length);
                            if (index == -1)
                                continue;
    ...
    while (1)
    {
        int32_t found_condensation = 0;
        for (int32_t i = 0; i < snapshot_counter; i++)
        {
...
int32_t cmp_state_search_a(const void *a, const void *b)
{
    STATE_SEARCH_STR *itemA = *(STATE_SEARCH_STR **)a;
    STATE_SEARCH_STR *itemB = *(STATE_SEARCH_STR **)b;

    int32_t valueA = itemA->elapsed + itemA->estimated;
    int32_t valueB = itemB->elapsed + itemB->estimated;
 ...
 * \param elist ENTRY*                  entry list
 * \param entry_count int32_t               entry count
 *\param first_nonperma_chunk_index int32_t index of the first chunk the entering entries are in
 * \param perma_chunk_count int32_t         number of permaloaded chunks (so perma chunks and entries arent involved in state eval etc)
 * \return void
 */
void build_state_search_solve(ENTRY *elist, int32_t entry_count, int32_t first_nonperma_chunk, int32_t perma_chunk_count)
{

    int32_t key_length, ll_count;
    ENTRY *temp_elist;
    ...
    int32_t temp_max, temp_eval, min = 0x7FFFFFFF;
    printf("Printing status line every time new best is reached, or every %d iterations\n", iter_print);
    for (int64_t iter = 0; (!heap_is_empty(*heap)); iter++)
    {
        ...
        int32_t end_index = build_state_search_str_chunk_max(top, key_length);
        for (uint32_t i = first_nonperma_chunk; i < (unsigned)end_index; i++)
        {
            // dont merge into empty chunks
            if (build_state_search_is_empty_chunk(top, i, key_length))
                continue;

            for (uint32_t j = first_nonperma_chunk; j < i; j++)
            {
