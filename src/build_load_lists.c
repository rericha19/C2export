    ...
    for (int32_t i = 0; i < entry_count; i++)
    {
        if (build_entry_type(elist[i]) == ENTRY_TYPE_ZONE)
        {
            ...
            for (int32_t j = 0; j < cam_count; j++)
            {
                ...
                for (int32_t l = 0; l < mus_deps.count; l++)
                {
 ...
 * \param is_daw bool                   is draw list instead of load list?
 * \return void
 */
    ...
    for (int32_t i = 0; i < cam_length && sublist_index < draw_list2.count; i++)
    {
        ...
        if (draw_list2.array[sublist_index].index == i)
        {
            if (draw_list2.array[sublist_index].type == 'B')
                for (int32_t m = 0; m < draw_list2.array[sublist_index].list_length; m++)
                {
...
void build_add_collision_dependencies(LIST *full_list, int32_t start_index, int32_t end_index, uint8_t *entry, DEPENDENCIES collisions, ENTRY *elist, int32_t entry_count)
{
    ...
    for (int32_t x = 0; x < neighbours.count; x++)
    {
        ...
        for (int32_t i = 0; i < count + 2; i++)
        {
            ...
            for (int32_t j = 0; j < collisions.count; j++)
